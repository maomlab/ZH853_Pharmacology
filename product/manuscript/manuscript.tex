...
structure of the MOR--ZH853--Gi--scFv16 complex: (1) \emph{what, if anything, is structurally
...
are shown in Figures~\ref{fig:overview} and~\ref{fig:pocket}.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.78\linewidth]{figures/fig6_molstar_overview.png}
  \caption{The experimentally characterized MOR--ZH853--Gi--scFv16 cryo-EM complex (Mol* render), in
  the canonical GPCR orientation with the extracellular face up and the intracellular G-protein
  interface down: \uOR{} receptor 7TM bundle (gold, top) with ZH853 (green sticks) in the extracellular
  orthosteric pocket, coupling to G$\alpha$i (orange), G$\beta$ (teal), and G$\gamma$ (purple) below,
  with the scFv16 fragment (magenta).}
  \label{fig:overview}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/fig7_molstar_pocket.png}
  \caption{Orthosteric pocket of the ZH853 complex (Mol* render): ZH853 (yellow ball-and-stick) with
  key receptor residues (blue) as sticks, labeled by Ballesteros-Weinstein number. Salt bridges
  (orange) to Asp3.32 and ECL2 Glu231 and H-bonds (blue) to Tyr7.43 and Asn2.63 are drawn as distance
  lines; the receptor cartoon is faded for context.}
  \label{fig:pocket}
\end{figure}
