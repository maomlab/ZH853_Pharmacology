...
HID/HIE tautomers are set from the local hydrogen-bond network (\textbf{HIE} for H173; \textbf{HID}
for H225, H299/6.52 and H321/7.36) because H6.52 and H7.36 line the pocket and a mis-assigned tautomer
would perturb the interactions under study. The assignment is written into the residue name at the
preparation stage rather than carried implicitly, since \texttt{tleap} maps a residue still named
\texttt{HIS} to HIE irrespective of the tautomer determined upstream --- silently, and for three of
these four sites wrongly. The truncated receptor termini
are capped with neutral acetyl/\emph{N}-methylamide groups rather than charged termini, which would be
artifacts on an internal transmembrane fragment.

The construct contains twelve cysteines but only one disulfide: the class-A-conserved
\textbf{Cys142--Cys219} bond, from TM3 position 3.25 to ECL2, which tethers the loop across the mouth
of the orthosteric pocket. The assignment is unambiguous on geometry alone --- these two S$\gamma$
atoms are 2.02\,\Ang{} apart, whereas the next-closest S$\gamma$ pair in the model is 8.07\,\Ang{}
away, so the remaining ten cysteines are
left reduced. This particular bond is load-bearing for the present work rather than incidental --- the
interaction that distinguishes ZH853 from every other agonist in the comparator set is a salt bridge to
Glu231, which lies \emph{in ECL2}, and an unrestrained loop that lifts off the pocket would make exactly
the contact we set out to quantify unmeasurable. We therefore do not infer the bond from deposited
connectivity records or residue names. At assembly the pair is detected geometrically
(S$\gamma$--S$\gamma$ \lteq2.5\,\Ang), both residues are renamed CYS\ra CYX so that ff19SB does not
build a thiol hydrogen onto a sulfur that is about to take a second bond, and the linkage is declared
to \texttt{tleap} [32] explicitly rather than left to distance-based auto-detection. One implementation
detail is recorded here because it is a \emph{silent} failure mode rather than a diagnosable one:
\texttt{loadpdb} renumbers every residue sequentially from 1 across the \emph{entire} assembled system
--- receptor first, then lipids, water and ions --- so the human OPRM1 numbering used throughout this
paper has ceased to exist by the time the bond is declared, Cys142 and Cys219 becoming residues 74 and
151 for the 69--349 construct. A hardcoded index pair would bond two arbitrary residues, plausibly a
lipid to a water, and neither \texttt{tleap} nor the subsequent energy minimization would necessarily
flag it. The indices are instead derived from the packed coordinates at build time, which also keeps
them correct under terminal capping or any change of construct boundaries.

ZH853 is a macrocyclic peptide with D-amino acids and a
...
\textbf{3.7 Membrane placement and force field.} The receptor is placed in the bilayer by the standard
\textbf{OPM/PPM transfer-energy method} [19, 29]. Because our cryo-EM model is not itself in OPM, we
transfer the placement structurally: our receptor is superposed (C$\alpha$ Kabsch, RMSD 1.1\,\Ang) onto
the OPM-oriented MOR--Gi--DAMGO reference (6DDF), whose membrane boundaries (DUM pseudo-atoms) give a
\textbf{hydrophobic thickness of 31.4\,\Ang} --- consistent with OPM across active MOR structures
(31--35\,\Ang; 4DKL 32.0$\pm$1.0\,\Ang) and adopted as the build target. We do \emph{not} derive the
membrane from the resolved lipids: the 3 cholesterols bind at site-specific motifs, each spans about one
leaflet, and 3 molecules fix the midplane only to $\sim$2\,\Ang. Two features of our own map corroborate
the OPM placement (Figure~\ref{fig:membrane}): the \textbf{Trp/Tyr aromatic girdle} --- interfacial
residues that snorkel to the headgroup regions [30] --- forms two bands $\sim$30\,\Ang{} apart (interfaces
at $z\approx+13$ and $-17$\,\Ang), and the cholesterols span $\sim$33\,\Ang{} about the OPM midplane; both
bracket the 31.4\,\Ang{} slab. This thickness matches the experimental POPC hydrocarbon core
(2$D_c$=28.8\,\Ang [31]) plus the cholesterol condensing effect ($+2$--$4$\,\Ang{} for a POPC-like
bilayer), within a phosphate-to-phosphate span of $\sim$37--40\,\Ang. For a definitive per-structure
value the model can be submitted to the PPM~3.0 server. The receptor is embedded in a
POPC:cholesterol (9:1) membrane with PACKMOL-Memgen [33] (CHARMM-GUI Membrane Builder [20] is the
documented alternative) --- a
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{\textbf{Membrane placement and thickness validation.} \textbf{(A)} Side view (Mol* render) of
  the oriented \uOR{} (grey cartoon) with the 3 modeled cholesterols (84 atoms; orange, ball-and-stick)
  and ZH853 (green). The \textbf{tan band is the inferred hydrophobic core} (\S3.7), drawn behind the
  receptor: the transmembrane helices span it and the cholesterols sit within it, while the intracellular
  loops fall below and ZH853 sits at the extracellular boundary. \textbf{(B)} Placement cross-check along
  the membrane normal $z$: receptor C$\alpha$ (grey), cholesterols (orange), ZH853 (green), and the
  Trp/Tyr aromatic girdle (purple diamonds; interfaces dotted). The \textbf{OPM slab} (yellow band,
  31.4\,\Ang, from the 6DDF superposition) is bracketed by the aromatic girdle ($\sim$30\,\Ang) and the
  cholesterol span ($\sim$33\,\Ang, dashed), and matches the experimental POPC hydrocarbon core
  (2$D_c$=28.8\,\Ang); ZH853 localizes to the extracellular leaflet, confirming the orientation.}
  \label{fig:membrane}
\end{figure}
...
\bibitem{ppm3} Lomize AL, Todd SC, Pogozheva ID. Spatial arrangement of proteins in planar and curved
  membranes by PPM 3.0. \emph{Protein Sci} 2022;31:209.
\bibitem{killian2000} Killian JA, von Heijne G. How proteins adapt to a membrane--water interface
  (Trp/Tyr interfacial ``aromatic belt''). \emph{Trends Biochem Sci} 2000;25:429.
\bibitem{kucerka2011} Ku\v{c}erka N, Nieh M-P, Katsaras J. Fluid phosphatidylcholine bilayer structural
  parameters as a function of temperature and chain length. \emph{Biochim Biophys Acta} 2011;1808:2761.
\bibitem{ambertools} Case DA, et al. AmberTools. \emph{J Chem Inf Model} 2023;63:6183.
\bibitem{packmolmemgen} Schott-Verdugo S, Gohlke H. PACKMOL-Memgen: a simple-to-use, generalized
  workflow for membrane-protein--lipid-bilayer system building. \emph{J Chem Inf Model} 2019;59:2522.
\end{thebibliography}
