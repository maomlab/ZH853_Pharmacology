...
Run: ``python src/02.07.00_analog_poses.py``  (or ``make prep-analogs-pose``).
"""
...
class _Tee:
    """Duplicate a stream into a buffer, so the terminal output can also be filed verbatim."""
...
def provenance(repo) -> list[str]:
    """What would be needed to explain, later, exactly what produced these files."""
    def git(*args):
        try:
    ...
    return [
        ...
        f"python    : {platform.python_version()}   rdkit {rdkit.__version__}",
        f"settings  : N_CONFS={N_CONFS} TETHER_K={TETHER_K} BOND_RANGE_A={BOND_RANGE_A} "
        f"CLASH_A={CLASH_A} MCS_TIMEOUT_S={MCS_TIMEOUT_S}",
    ]
...
def main() -> int:
    """Run the generation with stdout+stderr teed, then file the transcript."""
    ...
    transcript = "".join(tee_out.buf) + "".join(tee_err.buf)

    lig_dir = paths.INTERMEDIATE / "02.04.00_ligand"
    ori_dir = paths.INTERMEDIATE / "02.05.00_oriented"
    head = provenance(paths.ROOT)

    # 1. Plain transcript beside the generated files, as a record of this specific run.
    log = lig_dir / "02.07.00_analog_poses.log"
    log.write_text("\n".join(head) + "\n\n" + transcript)

    # 2. Dated Markdown in product/, matching what every other prep step writes
    #    (02.01.00_prep_assessment_*, 02.03.00_receptor_prep_*, 02.04.00_ligand_prep_* ...), so
    #    the record is discoverable where a reader already looks. Both files are git-ignored --
    #    product/*_YYYYMMDD.* is a regenerable snapshot by this repo's convention, the tracked
    #    deliverables being product/manuscript/* -- so this documents a run, it does not archive
    #    it. Copy the .md into the manuscript if a specific run needs to be cited.
    today = f"{date.today():%Y%m%d}"
    md = [f"# Analog poses by scaffold transfer — {today}", "",
          "Generated by `src/02.07.00_analog_poses.py` (`make prep-analogs-pose`). ZH850/ZH831/ZH809",
          "inherit the ZH853 crystallographic binding mode by constrained embedding on the maximum",
          "common substructure; only atoms outside it are rebuilt and relaxed.", "",
          "```", *head, "```", ""]
    if rows:
        md += ["| analog | scaffold inherited | rebuilt | MMFF vs template | scaffold drift | closest receptor contact |",
               "|--------|--------------------|---------|------------------|----------------|--------------------------|"]
        for r in rows:
            md.append(f"| {r['name']} | {r['n_core']}/{r['n_tot']} ({r['frac']:.0f} %) | "
                      f"{r['n_new']} | {r['mmff']:.1f} kcal/mol ({r['vs_template']:+.0f}) | "
                      f"{r['drift']:.2f} Å | {r['clash']:.2f} Å |")
        md += ["", "## Output files (sha256, first 16 hex)", ""]
        for r in rows:
            name = r["name"]
            for f in (lig_dir / f"{name}_prepared.sdf", ori_dir / f"complex_{name}_oriented.pdb"):
                if f.exists():
                    md.append(f"- `{f.relative_to(paths.ROOT)}` — `{sha256(f)}`")
    md += ["", "## Transcript", "", "```", transcript.rstrip(), "```", ""]
    report = paths.ensure_dir(paths.PRODUCT) / f"02.07.00_analog_poses_{today}.md"
    report.write_text("\n".join(md) + "\n")

    print(f"\nrecorded: {log.relative_to(paths.ROOT)}")
    print(f"          {report.relative_to(paths.ROOT)}")
    return rc
...
def _generate():
    ...
    if not cx_path.exists():
        ...
        print("  This is the only input this step needs (that and rdkit). Two ways to get it:",
              file=sys.stderr)
        print("   - in the LOCAL analysis env:  make prep-receptor-rebuild prep-receptor-orient", file=sys.stderr)
    ...
    for name in ANALOGS:
        ...
        rows.append({"name": name, "n_core": n_core, "n_tot": mol.GetNumAtoms(), "frac": frac,
                     "n_new": n_new, "n_heavy": n_heavy, "clash": clash,
                     "mmff": energy, "vs_template": energy - baseline, "drift": drift})

    print()
    print(f"wrote to {lig_dir.relative_to(paths.ROOT)}/ and {ori_dir.relative_to(paths.ROOT)}/:")
    for r in rows:
        print(f"  {r['name']}_prepared.sdf + complex_{r['name']}_oriented.pdb")
    if failed:
        print(f"\n{failed} analog(s) failed.", file=sys.stderr)
        return 1, rows

    bad = [r for r in rows if r["clash"] < CLASH_A]
    if bad:
